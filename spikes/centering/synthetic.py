"""Synthetic bordered-card generator for testing the centering measurement.

Real graded-card photos with ground-truth border pixels are scarce and noisy. A
synthetic card lets us assert the *math* against an exact, known centering offset: we
draw an outer card on a contrasting background and an inner art panel whose four border
widths we choose, then check the measurement recovers them. This validates the algorithm
independently of a labelled dataset — the standard way to pin geometric CV before real
data exists.
"""
